...
test_df = pd.read_csv(r'D:\datascience\Sec_homework\dataset\test1.csv')

# 原本的欄位包含 star_sign、phone_os 與 sleepiness，現已移除
...
def detect_outliers_iqr(df, cols):
    condition = pd.Series([True] * df.shape[0])
    for col in cols:
...
def cap_outliers_iqr(df, cols, ref_df=None):
    df_capped = df.copy()
    if ref_df is None:
        ref_df = df
    for col in cols:
...
train_df = detect_outliers_iqr(train_df, num_cols).reset_index(drop=True)
test_df = cap_outliers_iqr(test_df, num_cols).reset_index(drop=True)

# 轉換 gender (1→男, 2→女) 為二元標籤：0=男, 1=女
