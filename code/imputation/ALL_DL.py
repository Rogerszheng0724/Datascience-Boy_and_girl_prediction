...
def gain_imputation(df, batch_size=128, hint_rate=0.9, alpha=50, iterations=1500):
    ...
    參考: Yoon et al. (2018) "GAIN: Missing Data Imputation using Generative Adversarial Nets"
    ...
    # 建立 Discriminator 模型
    disc_input_dim = n_features * 2  # imputed data 與 hint matrix 串接
    discriminator = models.Sequential([
        layers.Dense(64, activation='relu', input_dim=disc_input_dim),
        layers.Dense(64, activation='relu'),
        layers.Dense(n_features, activation='sigmoid')
    ])
...
def autoencoder_imputation(df, epochs=200, batch_size=128):
    ...
    調整訓練 epochs 為 200，並加入 Dropout 層以豐富模型表現
    """
    X = df.values.astype(np.float32)
    M = 1 - np.isnan(X)
...
def rnn_imputation(df, epochs=200, batch_size=128):
    ...
    調整訓練 epochs 為 200，並在 LSTM 層中加入 dropout 參數
    """
    X = df.values.astype(np.float32)
    M = 1 - np.isnan(X)
    col_mean = np.nanmean(X, axis=0)
...
# ===========================
def main():
    # 讀取資料，使用 index_col=0 避免讀入多餘的索引欄位
    input_file = r"D:\datascience\Sec_homework\dataset\train.csv"
    df = pd.read_csv(input_file, index_col=0)
    
    # -------------------------
    # 前處理：針對 phone_os 欄位的轉換
    # -------------------------
    df['phone_os'] = df['phone_os'].str.lower().str.strip().map({'apple': 1, 'android': 2})
    
    # -------------------------
    # 分離 self_intro 欄位，並刪除 gender（預測欄位）
    # -------------------------
    ...
    # 各方法評估（以原始尺度數據比較）
    # -------------------------
    eval_knn = evaluate_imputation(df_clean, df_knn, mask_info)
    eval_mice = evaluate_imputation(df_clean, df_mice, mask_info)
    eval_em = evaluate_imputation(df_clean, df_em, mask_info)
    eval_hotdeck = evaluate_imputation(df_clean, df_hotdeck, mask_info)
    eval_nmf = evaluate_imputation(df_clean, df_nmf, mask_info)
    eval_gain = evaluate_imputation(df_clean, df_gain, mask_info)
    eval_autoencoder = evaluate_imputation(df_clean, df_autoencoder, mask_info)
    eval_rnn = evaluate_imputation(df_clean, df_rnn, mask_info)
    
    print("KNN 補值評估結果 (分欄位):")
    for col, metrics in eval_knn.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nMICE 補值評估結果 (分欄位):")
    for col, metrics in eval_mice.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nEM 補值評估結果 (分欄位):")
    for col, metrics in eval_em.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nHot Deck 補值評估結果 (分欄位):")
    for col, metrics in eval_hotdeck.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nNMF 補值評估結果 (分欄位):")
    for col, metrics in eval_nmf.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nGAIN 補值評估結果 (分欄位):")
    for col, metrics in eval_gain.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nAutoencoder 補值評估結果 (分欄位):")
    for col, metrics in eval_autoencoder.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
    
    print("\nRNN 補值評估結果 (分欄位):")
    for col, metrics in eval_rnn.items():
        print(f"{col}: MSE = {metrics['MSE']:.4f}, RMSE = {metrics['RMSE']:.4f}")
