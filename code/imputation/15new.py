...
def validate_imputed_values(df):
    ...
    # 對 star_sign 進行四捨五入並限制在 1 到 12
    df_valid['star_sign'] = np.round(df_valid['star_sign']).clip(1, 12)
    # 對 sleepiness 進行四捨五入並限制在 1 到 5
...
def gain_imputation(df, batch_size=128, hint_rate=0.9, alpha=50, iterations=1500):
    """
    修改後的 GAIN 補值方法（資料需為正規化後的數據）
    使用增加層數、LeakyReLU、BatchNormalization 與 Dropout 的架構
    """
    X = df.values.astype(np.float32)
    M = 1 - np.isnan(X)  # 掩膜矩陣，缺失值為 0
    # 對缺失值進行隨機均勻填補（資料已正規化至 [0,1]）
    X_filled = np.where(M == 1, X, np.random.uniform(0, 1, X.shape).astype(np.float32))
    n_samples, n_features = X.shape
    input_dim = n_features * 2  # 輸入為資料與 mask 的串接

    # 修改後的生成器：增加層數、使用 LeakyReLU、BatchNormalization 與 Dropout
    ...
    for it in range(iterations):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # 生成器損失：同時考慮欺騙判別器與逼近真實值（透過 alpha 平衡）
...
def main():
    ...
    for gender_val in [1, 2]:
        print("\n========== Gender = {} ==========".format(gender_val))
        df_gender = df[df['gender'] == gender_val].copy()
        df_self_intro = df_gender['self_intro']
        df_for_imputation = df_gender.drop(['self_intro', 'gender'], axis=1)
        ...
        for col in df_clean.columns:
            sorted_methods = sorted(eval_methods.items(), key=lambda x: x[1][col]['RMSE'])
            best_method = sorted_methods[0][0]
            second_method = sorted_methods[1][0] if len(sorted_methods) > 1 else None
            best_rmse = sorted_methods[0][1][col]['RMSE']
            best_mse = sorted_methods[0][1][col]['MSE']
            second_rmse = sorted_methods[1][1][col]['RMSE'] if second_method else None
            summary_rows.append({
                '欄位': col,
                '最佳方法': best_method,
                'RMSE': best_rmse,
        ...
        print("\nGender {} 最終最佳方法評估表：".format(gender_val))
        print(summary_df)
